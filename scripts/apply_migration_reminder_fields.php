<?php
/**
 * Script para aplicar a migration de campos de lembretes na tabela orders
 */

require_once __DIR__ . '/../config/database.php';

echo "=== Aplicando Migration: reminder_fields_to_orders ===\n\n";
...
if (empty($missingColumns)) {
    ...
    if (!empty($indexes)) {
        echo "✅ Índice idx_orders_reminder já existe!\n";
        exit(0);
    } else {
...
// Ler a migration
$migrationFile = __DIR__ . '/../database/migrations/20251116_add_reminder_fields_to_orders.sql';
...
try {
    ...
    // Verificar índice
    $indexesAfter = fetchAll("SHOW INDEX FROM orders WHERE Key_name = 'idx_orders_reminder'");
    if (!empty($indexesAfter)) {
        echo "✅ Índice idx_orders_reminder criado com sucesso!\n";
    } else {
